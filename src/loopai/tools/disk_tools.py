...
决策引用:
    ...
    D-16: 沙箱执行边界——rm 操作限制在 /tmp/loopai-demo/ 内
...
用法::
    ...
    register_disk_tools(registry, working_dir="/tmp/loopai-demo")
"""
...
def register_disk_tools(
    registry: ToolRegistry,
    working_dir: str = "/tmp/loopai-demo",
) -> None:
    ...
    async def disk_rm(
        ...
        Returns:
            确认信息字符串，如 ``"已删除 /tmp/loopai-demo/tmp/temp.log"``。
